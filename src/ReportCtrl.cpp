...
#define REPORTDATA_MAX_NODATA	28
...
BOOL CReportData::GetSubItem(INT iSubItem, LPINT lpiImage, LPINT lpiOverlay, LPINT lpiCheck, LPINT lpiColor, LPTSTR lpszText, LPINT lpiTextMax, ULONG* lplProgress)
{
	INT i, iPos, iText;
	...
	lpsz = &lpsz[iPos];
	VERIFY(_stscanf(lpsz, _T("(%d,%d,%d,%d,%d,%u")
		, lpiImage, lpiOverlay, lpiCheck, lpiColor, &iText, lplProgress));
...
BOOL CReportData::SetSubItem(INT iSubItem, INT iImage, INT iOverlay, INT iCheck, INT iColor, LPCTSTR lpszText, ULONG lProgress)
{
	if(!InsertSubItem(iSubItem, iImage, iOverlay, iCheck, iColor, lpszText, lProgress))
		return FALSE;
...
BOOL CReportData::InsertSubItem(INT iSubItem, INT iImage, INT iOverlay, INT iCheck, INT iColor, LPCTSTR lpszText, ULONG lProgress)
{
	INT i, iPos, iText;
	...
	TCHAR szProgress[16];
	_ultot(lProgress, szProgress, 10);
	TCHAR sz[48+REPORTCTRL_MAX_TEXT];
	_stprintf(sz, _T("(%d,%d,%d,%d,%d,%s)%s%c")
		, iImage
		, iOverlay
		, iCheck
		, iColor
...
#pragma message("LET OP: GEEN CORRECTE hInst IN REPORTCTRL()!")
	HINSTANCE hInst = 0;//::AfxGetInstanceHandle();
	...
	m_strNoItems = _T("No directory selected");
...
BOOL CReportCtrl::GetItem(LPRVITEM lprvi)
{
	...
	item.rdData.GetSubItem(lprvi->iSubItem, &lprvi->iImage, &lprvi->iOverlay, &lprvi->iCheck, &lprvi->iTextColor, lprvi->lpszText, &iTextMax, &lprvi->lProgress);
	...
	if (lprvi->lProgress != -1)
		lprvi->nMask |= RVIM_PROGRESS;
...
BOOL CReportCtrl::SetItem(LPRVITEM lprvi)
{
	...
	VERIFY(item.rdData.InsertSubItem(rvi.iSubItem, rvi.iImage, rvi.iOverlay, rvi.iCheck, rvi.iTextColor, rvi.lpszText, rvi.lProgress));
...
INT CReportCtrl::DefineSubItem(INT iSubItem, LPRVSUBITEM lprvs, BOOL bUpdateList)
{
	...
	try
	{
		...
		VERIFY(m_itemEdit.rdData.InsertSubItem(iSubItem, -1, -1, -1, -1, NULL, -1));

		if(!(m_dwStyle&RVS_OWNERDATA))
		{
			INT iItems = m_arrayItems.GetSize();
			for(i=0;i<iItems;i++)
				VERIFY(m_arrayItems[i].rdData.InsertSubItem(iSubItem, -1, -1, -1, -1, NULL, -1));
		}
...
BOOL CReportCtrl::SortItems(INT iSubItem, BOOL bAscending)
{
	...
	else
	{
		...
		MoveItem(_T("Total"), iItemCount-1);
		MoveItem(_T("­­­"), iItemCount-2);
...
INT CReportCtrl::CompareItems(INT iItem1, INT iSubItem1, INT iItem2, INT iSubItem2)
{
	...
	if(m_lpfnrvc == NULL)
	{
		...
		if(!(rvi1.nMask&RVIM_TEXT || rvi2.nMask&RVIM_TEXT))
		{
			...
			if ((rvi1.nMask&RVIM_PROGRESS || rvi1.lProgress))
			{
				if (rvi1.lProgress < rvi2.lProgress)
					return -1;
				else if (rvi1.lProgress > rvi2.lProgress)
					return 1;
				else
					return 0;
			}
		...
		// check for a number
		else if (isdigit(*szText1))
		{
			if (IsNumber(szText1) && IsNumber(szText2))
			{
				// remove any dots,comma's and stuff
				TCHAR szNum1[MAX_PATH],szNum2[MAX_PATH];
				GetNumber(szText1, szNum1);
				GetNumber(szText2, szNum2 );
				
				long d1=_ttol(szNum1),d2 = _ttol(szNum2);
				if (d1<d2)
					return -1;
				else if (d1 > d2)
					return 1;
...
CReportCtrl::ITEM& CReportCtrl::GetItemStruct(INT iItem, INT iSubItem, UINT nMask)
{
	...
	if(m_dwStyle&RVS_OWNERDATA)
	{
		...
		if(m_bUseItemCacheMap && lpItem != NULL)
		{
			...
			ULONG lProgress;
			if(lpItem->rdData.GetSubItem(iSubItem, &iImage, &iOverlay, &iCheck, &iColor, szText, &iTextMax, &lProgress))
				return *lpItem;
		}
		else
		{
...
BOOL CReportCtrl::LoadItemData(LPITEM lpItem, INT iItem, INT iSubItem, UINT nMask)
{
	...
	if(pWnd)
	{
		...
		INT lProgress = nmrvic.item.nMask&RVIM_PROGRESS ? nmrvic.item.lProgress:-1;

		if(iSubItem >= 0)
			lpItem->rdData.SetSubItem(iSubItem, iImage, iOverlay, iCheck, iColor, szText, lProgress);
...
void CReportCtrl::DrawItem(CDC* pDC, CRect rect, LPRVITEM lprvi)
{
	...
	if( !bResult )
	{
		...
		rect.left += (iWidth = DrawProgress(pDC, rect, lprvi)) ? iWidth+m_iSpacing : 0;
...
INT CReportCtrl::DrawProgress(CDC *pDC, CRect rect, LPRVITEM lprvi)
{
	INT iWidth=0;
	if (lprvi->nMask&RVIM_PROGRESS)
	{
		lprvi->nMask |= RVIM_LPARAM;
		VERIFY(GetItem(lprvi));
		if (lprvi->lParam == ID_LPARAM_STANDARDITEM)
		{

			double fPct = !m_lProgressTotal ? 100 : (double)lprvi->lProgress / (double)m_lProgressTotal;
			if (fPct>1)
				fPct=1;
			...
			vert [0] .x      = rect.left-tm.tmAveCharWidth;
			vert [0] .y      = rect.top + 2;
			vert [0] .Red    = 0x0000;
			vert [0] .Green  = 0xff00;
			vert [0] .Blue   = 0x0000;
			...
			vert [1] .x      = rect.left;
			vert [1] .y      = rect.bottom - 2; 
			vert [1] .Red    = (COLOR16)(0xff00*fPct);
			vert [1] .Green  = (COLOR16(0xff00*(1-fPct)));
			vert [1] .Blue   = 0x0000;
			vert [1] .Alpha  = 0x0000;

			if (fPct>0.004)
			{
				vert [1] .x  += (int)((rect.Width()-6*tm.tmAveCharWidth) * fPct);
			...
			CString szFmt;
			szFmt.Format( _T("%s%.f%%"), (fPct>0.004) ? _T(" ") : _T(""), fPct*100);
 						
			pDC->DrawText(szFmt, -1, rect, DT_LEFT|DT_END_ELLIPSIS|DT_SINGLELINE|DT_VCENTER);
		}
		//pDC->FillSolidRect(&rect, RGB(0,0,238));*/
	}
	return iWidth;
}
...
#pragma message("LET OP: GEEN CORRECTE hInst IN REPORTTIPCTRL()!")
	HINSTANCE hInst = 0;//AfxGetInstanceHandle();
...
BOOL CReportTipCtrl::Create(CReportCtrl* pReportCtrl)
{
	...
	return CreateEx( dwStyle
		, REPORTTIPCTRL_CLASSNAME
		, NULL,
		WS_BORDER|WS_POPUP,
		CRect(0, 0, 0, 0),
		pReportCtrl, 0, NULL
...
void CReportCtrl::SetProgressTotal(ULONG lTotal)
{
	m_lProgressTotal = lTotal;
}

void CReportCtrl::MoveItem(LPCTSTR szText, int iNewItemPos)
{
	RVFINDINFO rvfi;
	rvfi.nFlags = RVFI_TEXT;
	rvfi.lpszText = szText;

	int iItem = RVI_INVALID;

	BOOL bFoundTotal=FALSE;
	for (int iSubItem = 0 ; IsActiveSubItem(iSubItem) ; iSubItem++)
	{
		if (RVI_INVALID != (iItem = FindItem(&rvfi, iSubItem, -1)))
		{
			// als het item met szText was toegevoegd aan de lijst op positie
			// iNewItemPos dan moet iItem gelijk zijn aan iNewItemPos
			// deze positie veranderd niet
			ASSERT(iItem == iNewItemPos);

			for (int i=0;i<m_arrayRows.GetSize();i++)
			{				
				if (m_arrayRows[i] == iItem)
				{
					if (i == iNewItemPos)
					...
					if (i<iNewItemPos)
					{
						for (int j=i+1 ; j <= iNewItemPos ; j++)
							m_arrayRows[j-1] = m_arrayRows[j];
					}
					else
					{
						for (int j=i-1 ; j >= iNewItemPos ; j--)
...
int CReportCtrl::GetProgressColumn()
{
	RVITEM rvi;
	...
	for (rvi.iSubItem =0 ; IsActiveSubItem(rvi.iSubItem) ; rvi.iSubItem++)
	{
		for (rvi.iItem =0 ; rvi.iItem != iItemCount  ; rvi.iItem++)
		{
			VERIFY(GetItem(&rvi));
			if (rvi.lProgress>0L && rvi.lProgress != -1)
				return rvi.iSubItem;
		}
	}
	return -1;		
...
BOOL CReportCtrl::IsNumber(LPCTSTR szTxt)
{
	while (*(szTxt++) && !isalpha(*szTxt));
	return *(szTxt-1) == 0;
}

void CReportCtrl::GetNumber(LPCTSTR szNumber, LPTSTR szRet)
{
	szNumber-=1;
	while( *(++szNumber))
		if (isdigit(*szNumber))
