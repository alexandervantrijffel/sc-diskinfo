...
#define ID_LPARAM_STANDARDITEM 0x00f0
#define ID_LPARAM_SEPARATOR	   0x00f1
#define ID_LPARAM_TOTAL		   0x00f2
...
#define RVCF_SUBITEM_PROGRESS	0x08000000
...
typedef struct _RVITEM
{
	...
	_RVITEM() : nMask(0), iItem(RVI_INVALID), iSubItem(-1), lpszText(NULL), iTextMax(0), iTextColor(-1), iImage(-1), iCheck(-1), iBkColor(-1), nPreview(0), iIndent(-1), nState(0), lParam(0), iOverlay(0), lProgress(0) {};
...
public:
	...
	BOOL GetSubItem(INT iSubItem, LPINT lpiImage, LPINT lpiOverlay, LPINT lpiCheck, LPINT lpiColor, LPTSTR lpszText, LPINT lpiTextMax, ULONG* lplProgress);
	BOOL SetSubItem(INT iSubItem, INT iImage, INT iOverlay, INT iCheck, INT iColor, LPCTSTR lpszText, ULONG lProgress);

	BOOL InsertSubItem(INT iSubItem, INT iImage, INT iOverlay, INT iCheck, INT iColor, LPCTSTR lpszText, ULONG lProgress);
...
// Overrides
public:
	int GetProgressColumn();
	void SetProgressTotal(ULONG lTotal);
	virtual ~CReportCtrl();
...
protected:
	...
	ULONG m_lProgressTotal; // in KBytes
	...
	// Generated message map functions
protected:
	void GetNumber(LPCTSTR szNumber, LPTSTR szRet);
	BOOL IsNumber(LPCTSTR szTxt);
	void MoveItem(LPCTSTR szText, int iNewItemPos);
	INT DrawProgress(CDC* pDC, CRect rect, LPRVITEM lprvi);
	//{{AFX_MSG(CReportCtrl)
